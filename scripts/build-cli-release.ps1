...
& cargo build --manifest-path $manifestPath --release --bin link-world-cli
if ($LASTEXITCODE -ne 0) {
  throw "CLI release build failed with exit code $LASTEXITCODE."
}

$binaryPath = Join-Path $repo 'src-tauri/target/release/link-world-cli.exe'
...
$metadataPath = Join-Path $repo 'src-tauri/target/release/link-world-cli.build.json'
