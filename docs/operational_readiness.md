...
当前实现边界：capture submit/fetch 已写入 `logs/link-world.jsonl`，单文件 2 MiB、保留一份轮转；写入与支持包读取均复验字段和敏感 marker。AI、restore、migration、search maintenance 等关键流程尚未全部接入，因此 Week 5 日志验收仍未完成。
