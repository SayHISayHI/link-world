...
- 自动化门禁：运行 `npm run readiness:sprint5`，覆盖 Diagnostics 健康聚合与脱敏、有界日志校验和轮转、支持包确认/隐私/原子发布，以及 capture、AI、search、startup migration 和 restore 的 correlation/失败脱敏边界。
...
- startup migration 使用写入 guard/last-result 的 UUID 跨启动关联 started/prepared/running/succeeded/failed；覆盖成功迁移、running guard 阻断、损坏 guard fail-closed；新 migration backup ID、绝对路径和损坏控制文件内容不进入日志，legacy backup UUID 只允许作为 correlationId。
- restore 使用 marker `transactionId` 贯穿 prepare、重启、candidate install、validation、success/rollback，并写入 prepare result 与 last-result correlation；覆盖正常完成、moving-live 中断回滚、candidate 篡改 fail-safe 和损坏 marker，日志不含 target/safety backup ID、路径、候选内容或 raw error。
...
| W5-08 | capture、AI、search、migration 与 restore correlation | 各完成一次成功/失败 capture、成功/失败 AI enrichment、成功/取消/失败 search rebuild、对象 reindex、成功 startup migration、running guard 阻断、成功 restore、interrupted restore rollback 和 candidate 篡改，比较 job、domain event、IPC result、migration/restore control result、结构化日志和支持包摘要 | 每次流程共享一个稳定 UUID；search 使用 job UUID；migration 跨 guard/result/重启复用 UUID；restore 跨 prepare result/marker/last-result/重启复用 transaction UUID；不同操作不错误复用；AI event payload 只含 analysis id 或稳定 `ai.*` code；search/migration/restore 不含正文、query、索引内容、绝对路径或 raw error；新 migration backup ID 与 restore target/safety backup ID 不进入日志，legacy migration backup UUID 只允许作为 correlation；支持包只含允许字段 |
...
- 路线图中列出的计划内关键流程均已接入受限结构化日志和稳定 correlation id；capture、AI enrichment、search maintenance、startup migration 与 restore 已满足确定性自动化边界。该项的代码级覆盖已完成，但仍须通过本矩阵的真实 Windows、运行态轮转和支持交接证据，不能据此提前完成整个 Sprint 5。
