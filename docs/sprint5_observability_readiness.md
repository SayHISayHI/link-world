...
- capture submit/fetch 生命周期日志与 domain event 共用 correlation UUID、失败证据脱敏测试。
...
| W5-08 | capture correlation | 完成一次成功和一次失败 capture，比较 domain event、job payload、结构化日志和支持包摘要 | 每次流程各自共享一个稳定 UUID；不同 capture 不错误复用；支持包只含 id/type/time/error code 等允许字段 |
...
- 路线图中列出的关键流程均已接入受限结构化日志和稳定 correlation id；当前只有 capture 满足，因此该项仍未完成。
