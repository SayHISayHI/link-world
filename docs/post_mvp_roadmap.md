...
本阶段把“功能可用的 MVP”推进为“可以安全交给少量外部用户长期使用的本地产品”，同时开始验证 Node Tide 的核心差异：对内容价值进行有证据、有边界的评估，而不只是生成摘要。
...
| Sprint 1C 导航与列表边界 | 已实现，待 UI 回归 | 后端驱动 system/collection/topic/smart views；统一 typed scope 的列表与搜索；cursor 分页 |
| Sprint 1D 文档契约 | 已同步 | API、架构、安全、PRD、UI、数据库与本路线图 |
| Sprint 2 数据安全 | 执行中 | 备份/两阶段恢复/rollback、historical fixtures → 0007、启动迁移保护、启动恢复界面与便携导出已实现；readiness 自动化门禁已建立；真实 Windows 安装包故障矩阵待执行 |
...
| Sprint 9-10 Alpha 发布 | 自动化与运营基础完成，真实 Alpha 待执行 | Node Tide commit `c472b7a8` 已完成 22/22 自动门禁并生成 commit-bound MSI/NSIS/CLI、manifest 和 SHA-256；首轮三步引导、用户 README、邀请/反馈模板与 fail-closed 脱敏证据门禁已建立；真实 Windows 安装升级卸载、签名和 5-15 名用户观察仍待执行 |
| CLI 自动化界面 | 已实现，待最终 RC 矩阵 | 独立 `node-tide-cli.exe`、共享 service、OS runtime lock、schema v1 JSON/退出码、capture/AI/Evaluation idempotency、export/backup/search maintenance、completion、用户级安装脚本和 `readiness:cli` 已实现；签名、Defender、代理与用户 PATH 会话仍待发布候选实测 |
| Knowledge Organization | 已实现，待完整 UI 回归 | 独立 triage；用户 collection/topic；AI pending topic suggestions；后端计数；list/search 同 scope；revision 与 audit 边界 |
...
- Sidebar 由后端导航模型驱动，system view、collection、topic 和 smart view 均使用稳定 ID，不做纯视觉导航。
- 列表和搜索共享 typed scope 与 filter 解析，按 cursor 分页并提供 Load more。
...
- historical schema 到 latest（0007）的生成式 migration fixture。（已实现）
...
- Alpha 聚合门禁：former Link World commit `98c9b0f` 的 17/17 steps 仅作历史证据；Node Tide commit `c472b7a8` 已完成 22/22 steps，见 `docs/release_evidence/alpha_0.1.0_c472b7a8.md`。
- 发布候选组装：Node Tide 的 commit-bound MSI/NSIS/CLI、`unsigned_alpha` manifest 与 SHA-256 清单已生成并独立复算；真实安装、签名和第二台机器复算仍待完成。
...
- 5-15 名目标用户的邀请、反馈模板和数据处理说明。（`docs/alpha_feedback_playbook.md` 与根目录用户 README 已建立；真实邀请待执行）
- 空库提供保存、搜索、Evaluation 三步首轮引导；AI 配置是可选入口，不阻断无模型本地闭环。（已实现并覆盖组件/E2E）
- P0/P1 缺陷响应和发布停止条件，以及匿名 schema v1 证据契约。（已定义）
- 核心漏斗：成功启动、首次保存、首次搜索、首次 AI 分析或正常无模型降级、首次 Evaluation。（观察模板与聚合门禁已实现；真实观察待完成）
- 下一阶段决策记录只允许一个 primary priority。（模板与 fail-closed 校验已实现；需真实证据后填写）
...
- Week 9/10 发布候选需运行 `npm run readiness:alpha`，并在发布候选时附加 Sprint 2/3/5 readiness JSON、安装包 manifest/checksum 和安全审计处置；开始真实邀请后还必须运行 `scripts/alpha-feedback-readiness.ps1` 验证脱敏 Alpha 证据。
