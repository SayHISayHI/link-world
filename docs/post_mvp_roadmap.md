...
| Sprint 4 搜索质量 | 执行中 | FTS 字段权重、secret snippet 抑制、Library filter 组合搜索、索引一致性检查和可重复大库搜索基准入口已实现；搜索空态/失败态/重建进度待实现 |
...
- 索引一致性检查与按对象/全库重建。（missing/stale/orphaned/duplicate 只读检查已实现；rebuild/reindex 已有入口）
