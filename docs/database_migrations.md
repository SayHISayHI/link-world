...
- 0003 → latest：验证非默认 `anthropic_messages` 不回退，并依次新增 0004 Evaluation runtime identity/version columns 与 0005 privacy-bounded trace table/索引。
- 0001 legacy Evaluation row → latest：验证 request/correlation 保持 NULL，plan/input/output schema version 均为 1，artifact 和历史 verdict 不丢失；legacy run 不强制伪造 trace。
