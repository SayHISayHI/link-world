...
- 支持包必须显式确认、固定写入本机应用目录且不自动上传；导出 JSON 不含正文、raw job/audit/plugin/domain-event payload、URL query/fragment、credential reference、embedding 或本机绝对路径；只允许 payload-free correlation summary。runtime logs 未实现时必须标记为 `not_collected`。
