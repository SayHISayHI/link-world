...
- Prompt Evaluator 完全本地执行并把 prompt 作为 untrusted data；原 prompt 只在本地 immutable Evaluation snapshot/output/artifact 中保留，不进入 trace、structured log 或 support bundle。generated tests 和 improvement diff 禁止复制 credential value、cookie、token、session、本机路径或用户正文片段；只允许变量名与固定 synthetic fixture。
