...
- Full search-index rebuild uses a staged FTS table (`knowledge_fts_rebuild`) and publishes only through an atomic final swap. Progress is persisted in `background_jobs.payload_json` with `expectedObjects`, `indexedObjects`, `stage` and `cancellable`. User cancellation is honored before `finalizing`; once finalizing begins, cancellation is disabled to avoid exposing a partially swapped index. Cancelled rebuilds preserve the previously published `knowledge_fts`.
